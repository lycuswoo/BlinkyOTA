...
static int ledGreenGpioFd = -1;
static int ledBlueGpioFd = -1;
static int ledWork = 0; //0: Green, 1: Blue
...
static void LedTimerEventHandler(EventData* eventData)
{
	...
	if (ledWork == 1)
	{
		GPIO_SetValue(ledGreenGpioFd, GPIO_Value_High);
		// Check for LED
		GPIO_Value_Type newLedState;
		int result = GPIO_GetValue(ledBlueGpioFd, &newLedState);
		if (result != 0) {
		...
		if (newLedState == GPIO_Value_High)
			GPIO_SetValue(ledBlueGpioFd, GPIO_Value_Low);
		else
			GPIO_SetValue(ledBlueGpioFd, GPIO_Value_High);
	...
	else //default
	{
		...
		// Check for LED
		GPIO_Value_Type newLedState;
		int result = GPIO_GetValue(ledGreenGpioFd, &newLedState);
		if (result != 0) {
			Log_Debug("ERROR: Could not read button GPIO: %s (%d).\n", strerror(errno), errno);
			terminationRequired = true;
			return;
		...
		if (newLedState == GPIO_Value_High)
			GPIO_SetValue(ledGreenGpioFd, GPIO_Value_Low);
		else
			GPIO_SetValue(ledGreenGpioFd, GPIO_Value_High);
	}
...
int main(void)
{
	...
	// Open Led Green GPIO as output
	ledGreenGpioFd = GPIO_OpenAsOutput(9, GPIO_OutputMode_PushPull, GPIO_Value_High);
	if (ledGreenGpioFd < 0) {
		Log_Debug(
	...
	// Open Blue Green GPIO as output
	ledBlueGpioFd = GPIO_OpenAsOutput(10, GPIO_OutputMode_PushPull, GPIO_Value_High);
	if (ledBlueGpioFd < 0) {
		Log_Debug(
			"Error opening GPIO: %s (%d). Check that app_manifest.json includes the GPIO used.\n",
			strerror(errno), errno);
